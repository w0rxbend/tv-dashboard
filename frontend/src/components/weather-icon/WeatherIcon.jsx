...
import { createWeatherIconAnimation } from './weatherIconMotion';
...
function ClearIcon(props) {
  ...
  createWeatherIconAnimation(() => props.animated, () => {
...
function HeavyRainIcon(props) {
  ...
  // 11 heavy diagonal lines in 2 rows
  const HL = [
...
function ThunderstormIcon(props) {
  ...
  createWeatherIconAnimation(() => props.animated, () => {
    ...
    timeline.to([boltG, bloom], { autoAlpha: 1, duration: 0.04 })
...
export default function WeatherIcon(props) {
  ...
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <Switch fallback={<CloudyIcon animated={animated()}/>}>
        <Match when={props.condition === 'clear'}>         <ClearIcon animated={animated()}/> </Match>
        <Match when={props.condition === 'partly_cloudy'}> <PartlyCloudyIcon animated={animated()}/> </Match>
        <Match when={props.condition === 'cloudy'}>        <CloudyIcon animated={animated()}/> </Match>
        <Match when={props.condition === 'rainy'}>         <RainyIcon animated={animated()}/> </Match>
        <Match when={props.condition === 'heavy_rain'}>    <HeavyRainIcon animated={animated()}/> </Match>
        <Match when={props.condition === 'thunderstorm'}>  <ThunderstormIcon animated={animated()}/> </Match>
        <Match when={props.condition === 'snowy'}>         <SnowyIcon animated={animated()}/> </Match>
        <Match when={props.condition === 'windy'}>         <WindyIcon animated={animated()}/> </Match>
        <Match when={props.condition === 'foggy'}>         <FoggyIcon animated={animated()}/> </Match>
      </Switch>
    </svg>
  );
}
